...
class Server:
    ...
    def _setup_routes(self) -> None:
        ...
        def get_domain_stops() -> Response:
            json_mapper: Callable[[Stop], dict[str, Any]] = lambda s: {'short_name': s.short_name, 'full_name': s.full_name}
        ...
        def get_domain_vehicles() -> Response:
            json_mapper: Callable[[Vehicle], dict[str, Any]] = \
                lambda v: {'vehicle_id': v.vehicle_id, 'type': v.model.kind, 'brand': v.model.brand, 'model': v.model.model
                           } if v.model else {'vehicle_id': v.vehicle_id}
